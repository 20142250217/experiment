...
def loadData(fileName, fileType='source'):
    ...
    for lines in text.split('\n'):
        line=jieba.lcut(lines,cut_all=False)
        for item in line:
    ...
    if fileType=='source':
        ...
        text = [[vocab2int[char] for char in jieba.lcut(line,cut_all=False)] for line in text.split('\n')]
    else:
        #如果是目标文件，在训练过程中需要添加EOS表示文本结束
        text = [[vocab2int[char] for char in jieba.lcut(line,cut_all=False)] + [vocab2int['<EOS>']] for line in text.split('\n')]
